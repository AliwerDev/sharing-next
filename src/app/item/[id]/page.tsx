...
function getImageUrl(url?: string) {
  if (!url)
    return "https://images.unsplash.com/photo-1485965120184-e220f721d03e?w=1200&q=80";
      ...
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
          ...
          <p className="text-muted-foreground mb-8">
            The listing you are looking for might have been deleted or does not
            exist.
          </p>
          <Link href="/discover">
  ...
  const handleRequestItem = async () => {
    try {
      ...
      toast.success("Request Sent!", {
        description:
          "Your request has been sent to the owner. You'll see their phone number once they approve.",
      });
    } catch (err: any) {
      toast.error(
        err.response?.data?.message ||
          "Failed to request item. Please try again."
      );
    }
  };
            ...
            <div className="mt-auto fixed bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-xl border-t border-border md:static md:p-0 md:bg-transparent md:border-t-0 md:backdrop-blur-none z-40">
              {isOwner ? (
                <Link href="/dashboard" className="w-full">
                  <Button className="w-full py-7 text-lg rounded-2xl shadow-xl hover-lift tactile-scale gap-2 bg-primary">
                    Manage This Listing (Dashboard){" "}
                ...
                <>
                  ...
                  {unveiledPhone && (
                    <a
                      href={`tel:${unveiledPhone}`}
                      className="w-full mt-3 block"
                    >
                      <Button className="w-full py-7 text-lg rounded-2xl shadow-xl hover-lift tactile-scale gap-2 bg-secondary text-secondary-foreground hover:bg-secondary/90">
                        <Phone className="w-6 h-6" /> Call Giver (
                        {unveiledPhone})
                      </Button>
                    </a>
                  )}
